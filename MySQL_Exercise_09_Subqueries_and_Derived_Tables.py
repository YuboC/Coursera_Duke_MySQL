...
get_ipython().run_cell_magic('sql', '', 'describe dogs')
...
get_ipython().run_cell_magic('sql', '', 'SELECT DistinctUUsersID.user_guid AS uUserID, d.user_guid AS dUserID,\nCOUNT(*) AS numrows\nFROM (SELECT DISTINCT u.user_guid\n      FROM users u ) AS DistinctUUsersID\nLEFT JOIN dogs d\n  ON DistinctUUsersID.user_guid=d.user_guid\nGROUP BY DistinctUUsersID.user_guid\nORDER BY numrows DESC;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT distinctDU.user_guid\nFROM (SELECT DISTINCT d.user_guid\n     FROM dogs d) AS distinctDU;')
...
get_ipython().run_cell_magic('sql', '', "SELECT DistinctUUsersID.user_guid AS uUserID, distinctDU.user_guid AS dUserID, count(*) AS numrows\nFROM (SELECT DISTINCT u.user_guid \n      FROM users u\n     WHERE u.user_guid='ce7b75bc-7144-11e5-ba71-058fbc01cf0b') AS DistinctUUsersID \nLEFT JOIN (SELECT DISTINCT d.user_guid\n     FROM dogs d) AS distinctDU\n  ON DistinctUUsersID.user_guid=distinctDU.user_guid\nGROUP BY DistinctUUsersID.user_guid\nORDER BY numrows DESC;\n")
...
get_ipython().run_cell_magic('sql', '', 'SELECT distinU.user_guid, distinD.dog_guid, distinD.breed\nFROM (SELECT DISTINCT u.user_guid\n      FROM users u\n      LIMIT 100) AS distinU\nLEFT JOIN (SELECT DISTINCT d.dog_guid, d.breed, d.user_guid\n     FROM dogs d) AS distinD\nON distinU.user_guid=distinD.user_guid\nGROUP BY distinU.user_guid;')
